...
export function CartItemProductSearch({ item, infoRes, customization, resto_id, isDishInCart }) {
...
useEffect(() => {
  ...
  return (
    ...
    //   <div className="flex">
    //     <Button onClick={() => setIsModalOpen(!isModalOpen)} className="h-full w-full  !py-0 px-0 bg-transparent hover:bg-transparent">
    //         <div className="relative rounded-[10px] w-full border-yellow-300 border-4 flex justify-between items-center ">
    //             <div
    //                 onClick={() => setSelectedItem(item)}
    //                 className="tab flex  border-pink-300 border-4   h-full w-full overflow-hidden p-1.5 pb-0 text-lg font-semibold rounded-[8px] cursor-pointer transition-colors"
    //                 >
    //                 <Avatar className="h-[100px] w-[100px] !rounded-[10px]">
    ...
    //                 </Avatar>
    //                             <div className='text-black flex ml-2 justify-between  '>
    //                                 <div className="w-full bg-[green] h-full flex flex-col  font-normal text-sm font-sans text-[#2F2F2F]">
    ...
    //                             </div>
    //             </div>
    //         <div className="tab flex flex-col border-blue-300 border-4   overflow-hidden p-1.5 pb-0 text-lg font-semibold rounded-[8px] cursor-pointer transition-colors ">         
    //               <button
    //                 type="button"
    //                 onClick={handleClick}
    //                 style={{ backgroundColor: "red" }}
    //                 className="text-white leading-0 w-[30px] h-[30px] flex items-center justify-center rounded-full"
    //               >
    //                 {isAdded ? (
    ...
    <div className="  flex justify-between items-stretch">
        <div className="flex flex-grow">
            ...
            <div className="mx-4 flex flex-col justify-evenly">
                {/* Aligner le nom du produit et l'icône d'ajout dans une ligne */}
                <div className="flex justify-between items-center">
                    <h1 className="ml-1 text-[#000] font-[Roboto] text-base not-italic font-semibold leading-[normal] tracking-[0.54px]">
                    {item.name.slice(0, 20)}
                    </h1>
                </div>

                {/* Aligner le timer et les étoiles sur une autre ligne */}
                <div className="flex items-center">
                    <RiTimerLine color="#A8A8A9" />
                    <h5 className="ml-1">15 min</h5>
                </div>

                {/* Aligner les étoiles et la note */}
                <div className="flex items-center justify-center">
                    ...
                    <MdOutlineStar color="yellow" />
                    <MdOutlineStar color="yellow" />
                    <h4 className="ml-2 text-sm">(4.5)</h4>
                </div>
            </div>
        </div>
        ...
        <div className="flex flex-col justify-between items-end">
            ...
            onClick={handleClick}
            style={{ backgroundColor: "red" }}
            className="mt-1 text-white leading-0 w-[30px] h-[30px] flex items-center justify-center rounded-full"
            >
            {isAdded ? (
                <CheckIcon className="w-4 h-4 text-white" />
            ) : (
            ...
            <p className="mb-4  text-[13px]" style={{ color: "red" }}>
