...
function CatandProduct({ slug , categories, dishes, setSelectedTab, selectedTab, resto, infoRes, tabel_id, customization, promo }) {
  ...
  <div className=''>
    <h1 className='text-base font-bold	 ml-6 mb-2'>Categories</h1>
         <div className={`  pl-4  overflow-auto scrollbar-hide  ${infoRes.language === 'ar' ? 'rtl' : 'ltr'}`} >
            ...
            {!filtredCategoy && categoriesWithOrder.map((item) => (
              <div key={item.id} className="relative  rounded-xl border-gray-300 border inline-block">
                <div
                     ...
                     className={` tab flex items-center focus:bg-[red] focus:text-white  justify-center h-9 pl-1.5 pr-2.5 font-semibold rounded-[8px] cursor-pointer transition-colors`}
                   role="button"
                  aria-selected={selectedTab === item.name}
                  tabIndex={0}
                  >
                  <h2 className="text-[14px] mb-0  mx-2 whitespace-nowrap" >{item.name}</h2>
                </div>
              </div>
            ))}
          </div>
