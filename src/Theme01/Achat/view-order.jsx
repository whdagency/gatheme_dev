    ...
    return (
        <Dialog open={isOpen} onOpenChange={setIsOpen}>
            <DialogTrigger asChild>
                <Button className="bg-[#509DFF] self-end hover:bg-blue-700 text-white px-2 mr-[6px] mb-2">
                    See Your Order
                </Button>
            </DialogTrigger>
            <DialogContent className="max-w-[80%] gap-0 px-[20px] py-[23px] rounded-lg">
                <DialogHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                    <DialogTitle className="text-2xl tracking-wide font-bold text-gray-700">
                        Order: <span className="text-[#7D7D7D] tracking-tight text-lg font-medium">#{orderDetails.order.id}</span>
                    </DialogTitle>
                    ...
                    {orderDetails.dishes.map((item) => (
                        <div key={item.id} className="flex items-center space-x-1 mb-4">
                            ...
                            <div className="flex-1 self-baseline mt-2">
                                ...
                                <p className="text-sm mt-1 text-gray-500">{item.price} MAD</p>
                            </div>
                ...
                <div className="flex justify-between rounded-lg p-3 bg-[#E8EFF7] items-center">
                    <h3 className="text-md font-semibold text-[#024CA3]">Total</h3>
                    <p className="text-md font-bold text-[#024CA3]">{orderDetails.order.total} MAD</p>
                </div>
            </DialogContent>
        </Dialog>
    )
