...
export default function Achat({ resto_id, infoRes, customization, slug }) {
  ...
  const totalCost = cartItems.reduce(
    (total, item) => {
      // Calculate item cost based on quantity and price
      const itemCost = item.price * item.quantity;

      // Add selected prices if they exist
      const selectedPrices = item?.selectedPrices?.length > 0 ? item.selectedPrices.reduce((sum, price) => sum + price, 0) : 0;

      // Return the total cost
      return total + itemCost + selectedPrices;
    },
    0
  );
  ...
  console.log("The RestoInfosss => ", cartItems?.map(item => item?.selectedPrices.length));
  ...
  const fetchValues = async () => {
    try {
      ...
      console.log("The ID OF THE ORDER => ", data);
      if (result) {
  ...
  console.log("The Resulat of The Status ", status);
  ...
  return (
    <div className={`bg-white snap-y scrollbar-hide dark:bg-gray-950 p-2 pt-4 rounded-lg shadow-lg max-w-[620px] mx-auto ${direction === 'rtl' ? 'text-right' : 'text-left'}`} dir={direction}>
      <div className="flex flex-col  justify-center px-4 pb-2">
        {orderID != "" || orderID != null ? (<StepsBar status={status} currentStep={currentStep} infoRes={infoRes} complete={complete} cartItems={cartItems} canceled={canceled} />
        ) : <></>}
      </div>
      ...
      <div className="flex flex-col gap-4  snap-y  scrollbar-hide">
        <div className="flex items-center justify-between">
          <h2 className="text-lg font-semibold text-gray-900 dark:text-gray-50" >{t("achat.shoppingCart")}</h2>
          ...
          <button onClick={() => dispatch(removeAll())} className="text-black bg-white border border-gray-300 focus:outline-none hover:bg-gray-100 focus:ring-4 focus:ring-gray-100 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 dark:bg-gray-800 dark:text-white dark:border-gray-600 dark:hover:bg-gray-700 dark:hover:border-gray-600 dark:focus:ring-gray-700">
            {/* <h1 className='mix-blend-difference' style={{color: customization?.selectedTextColor}}> */}
            {t("achat.clearBtn")}
            {/* </h1> */}
          </button>
      ...
      <AlertDialog
        ...
        className={`${isArabic === 'ar' ? 'text-right ' : 'text-left'}`}
        dir={direction}
      >
        <AlertDialogContent
        ...
        >
          ...
          <AlertDialogFooter className="flex flex-row gap-4 justify-center">
            ...
            <Button autoFocus onClick={() => setOrderSuccessModalOpen(false)} variant="outline" >
              {t("menu.cancel")}
            </Button>
      ...
      <div className=" bg-gray-100 dark:bg-gray-800 p-4 rounded-lg">
        <div className='flex items-center gap-2'>
          ...
          <div className="flex-1">
            ...
            <p className="text-gray-500 dark:text-gray-400 text-sm">{subtotal + " " + infoRes.currency}</p>
